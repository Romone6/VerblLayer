...
export async function POST(request: Request) { try { const { organisationId, user } = await getDevContext(); requirePermission(user.role, "apps:manage"); const parsed = createAppSchema.safeParse(await request.json()); if (!parsed.success) return badRequest("Invalid app payload", parsed.error.flatten()); const app = await prisma.app.create({ data: { organisationId, name: parsed.data.name, type: parsed.data.type, baseUrl: parsed.data.base_url, authMethod: parsed.data.auth_method, executionMode: parsed.data.execution_mode, connectionStatus: ConnectionStatus.not_connected, metadataJson: { provider_key: parsed.data.provider_key ?? inferProviderKey(parsed.data.type) } as Prisma.InputJsonValue } }); return ok({ app }, 201); } catch (error) { return error instanceof Error && error.message.startsWith("Forbidden") ? forbidden(error.message) : serverError(error); } }
