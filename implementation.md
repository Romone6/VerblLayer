...
- 2026-07-17: rebrand completed: product, package, protocol headers, default Postgres identifiers, documentation, tests, and GitHub repository are renamed from VerblLayer to Callable. Existing trusted-proxy deployments must send `x-callable-*` headers and local environments must use the Callable Postgres connection string. The Compose volume keeps its legacy physical name to preserve existing local records. Locked install, Prisma generation, 44 unit tests, lint, TypeScript, and production build passed locally; protected-branch CI remains the database-backed proof.
